...
function NotFound() {
  return (
    <div className="w-full min-h-screen bg-[var(--color-bg)] flex items-center justify-center p-6 bg-[url(https://static.vecteezy.com/system/resources/previews/015/586/867/large_2x/overlay-distressed-concrete-texture-background-free-photo.jpg)] bg-cover bg-center relative selection:bg-[var(--color-accent)] selection:text-white">
      ...
      <div className="relative z-20 w-full max-w-md bg-[var(--color-surface)] p-8 md:p-10 rounded-2xl border border-[var(--color-border)] shadow-xl text-center space-y-6 animate-scale-up">
        ...
        {/* 404 Header */}
        <div>
          <h4 className="text-[10px] tracking-[0.5em] text-[var(--color-accent)] font-black uppercase mb-1">
            ERROR CODE // 404
          </h4>
          <h1 className="text-2xl md:text-3xl font-black tracking-widest text-[var(--color-text)] uppercase animate-slide-up">
            DROP EXPIRATION
          </h1>
        </div>
        ...
        <div className="pt-2">
          <Link 
            to="/" 
            className="inline-flex items-center text-white justify-center gap-3 w-full bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] active:scale-[0.98] font-black text-xs tracking-widest uppercase py-4 rounded-xl shadow-md transition-all cursor-pointer"
          >
